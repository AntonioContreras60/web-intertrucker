...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (move_uploaded_file($archivo['tmp_name'], __DIR__ . $ruta_archivo)) {
        ...
        $sql = "INSERT INTO multimedia_recogida_entrega (nombre_archivo, tipo_archivo, url_archivo, geolocalizacion, timestamp, tamano, categoria, porte_id, tipo_evento)
                VALUES ('$nuevo_nombre_archivo', '$tipo_archivo', '$ruta_archivo', '$geolocalizacion', NOW(), '{$archivo['size']}', '$categoria', '$porte_id', '$tipo_evento')";

        if (mysqli_query($conn, $sql)) {
            echo "El archivo ($tipo_archivo) se ha subido correctamente y se ha registrado.";
        } else {
            echo "Error al registrar los detalles del archivo: " . mysqli_error($conn);
        }
    } else {
