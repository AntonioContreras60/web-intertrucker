...
if (!isset($_POST['porte_id'])) {
    die("Error: Acceso no autorizado o datos incompletos (falta porte_id).");
}
...
try {
    // 1) Verificar que el porte existe y obtener su usuario_creador_id
    ...
    // 2) Obtener el admin_id del usuario creador del porte
    ...
    // 3) Obtener el admin_id del usuario actual (sesión)
    ...
    // 4) Insertar un registro en cambios_titularidad
    ...
    // 5) Verificar si ambos usuarios pertenecen a la misma empresa
    ...
    // Mostrar mensaje de éxito y botón "Volver"
    echo "<p>$mensaje</p>";
    echo "<p>Se registró el cambio en 'cambios_titularidad'.</p>";

    // Redirección a la página anterior, si existe
