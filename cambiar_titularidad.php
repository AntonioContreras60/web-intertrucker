...
try {
    ...
    // Mostrar mensaje de éxito y botón "Volver" a la página anterior
    // --------------------------------------------------------------
    echo "<p>$mensaje</p>";
    echo "<p>Se registró el cambio en 'cambios_titularidad'.</p>";

    // 1) Tomamos la URL previa (si existe)
    $referer = $_SERVER['HTTP_REFERER'] ?? '';

    // 2) Si está vacío, por defecto a 'portes_nuevos_recibidos.php'
    if (empty($referer)) {
        $referer = 'portes_nuevos_recibidos.php';
    }

    // 3) Añadir query param aleatorio para forzar recarga (evitar caché)
