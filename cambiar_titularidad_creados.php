...
// Verificar si el usuario está autenticado y si llega POST con porte_id
// (Podrías cambiar esta lógica para comprobar estrictamente $_SERVER['REQUEST_METHOD']=='POST')
if (!isset($_SESSION['usuario_id']) || !isset($_POST['porte_id'])) {
    die("Error: Acceso no autorizado o datos incompletos.");
}
...
try {
    // 2) Verificar que el porte existe
    ...
    // 5) Insertar el registro en cambios_titularidad (siempre)
    ...
    if ($admin_id_creador === $admin_id_sesion) {
        ...
        // === CASO PARCIAL: distinta empresa => no se actualiza usuario_creador_id
        // Se registra el cambio en cambios_titularidad y, opcionalmente, algo en ofertas_varios
    ...
    // Intentar volver a la URL previa
