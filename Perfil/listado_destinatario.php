...
$order_by   = isset($_GET['order_by']) ? $_GET['order_by'] : 'fecha_entrega';
...
$order_sql = $ordenables[$order_by] ?? 'p.fecha_entrega';
...
$estados = [];
$resEstados = $conn->query("SELECT DISTINCT estado_recogida_entrega FROM portes WHERE destinatario_usuario_id = $usuario_id ORDER BY estado_recogida_entrega");
...
<main>
  ...
  <h2 style="text-align:center;margin-bottom:var(--spacing-base);">
      Listado de Portes que Entran (Soy Destinatario)
  </h2>
  ...
  <form class="filtros" method="get" style="margin-bottom:18px;text-align:center;">
    ...
    <a href="listado_destinatario.php" class="btn btn-gray" style="margin-left:7px;">Limpiar</a>
  </form>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>
                <a href="?<?= http_build_query(array_merge($_GET,['order_by'=>'fecha_entrega','order_dir'=>(($order_by=='fecha_entrega'&&$order_dir=='ASC')?'DESC':'ASC')])) ?>">
                  Fecha Entrega <?= $order_by=='fecha_entrega' ? ($order_dir=='ASC'?'▲':'▼') : '' ?>
                </a>
              </th>
              <th>Local. Entrega</th>
              <th>
                <a href="?<?= http_build_query(array_merge($_GET,['order_by'=>'estado','order_dir'=>(($order_by=='estado'&&$order_dir=='ASC')?'DESC':'ASC')])) ?>">
                  Estado Entrega <?= $order_by=='estado' ? ($order_dir=='ASC'?'▲':'▼') : '' ?>
                </a>
              </th>
              <th>Tren</th>
              <th>CIF Camionero</th>
